...
// --- Helpers ---
const getCalculatedStatus = (startDateStr: string | undefined, endDateStr: string | undefined): 'upcoming' | 'active' | 'completed' => {
...
export function AdminDashboardPage() {
  ...
  // Changed to an array to handle multiple active elections
  const [activeElections, setActiveElections] = useState<Election[]>([]);
  // Changed to a dictionary/record mapped by election_id
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // 1. Filter to get ALL active elections, not just the first one
      const currentActiveElections = allElections.filter(
        (e: Election) => getCalculatedStatus(e.start_date, e.end_date) === 'active'
      );
      
      setActiveElections(currentActiveElections);

      // 2. Fetch turnout for each active election concurrently
      if (currentActiveElections.length > 0) {
        const turnoutPromises = currentActiveElections.map(e => 
           votesApi.getTurnout(e.election_id).catch(() => null) // Catch individual errors so one failing doesn't break all
        );
        
        const turnoutResults = await Promise.all(turnoutPromises);
        
        // Map the results back to their respective election IDs
        const newTurnouts: Record<number, any> = {};
        currentActiveElections.forEach((election, index) => {
           if (turnoutResults[index]) {
             newTurnouts[election.election_id] = turnoutResults[index];
           }
        });
  ...
  const activeCount = elections.filter(e => getCalculatedStatus(e.start_date, e.end_date) === 'active').length;
  const upcomingCount = elections.filter(e => getCalculatedStatus(e.start_date, e.end_date) === 'upcoming').length;
  const completedCount = elections.filter(e => getCalculatedStatus(e.start_date, e.end_date) === 'completed').length;
        ...
        <div className="p-6 lg:p-10 space-y-8">
          ...
          <header className={`flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/5 opacity-0 ${mounted ? 'animate-enter' : ''}`}>
             ...
             <div className="flex flex-col sm:flex-row gap-3">
                 <div className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md flex flex-col items-center sm:items-end min-w-35">
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 opacity-0 ${mounted ? 'animate-enter delay-100' : ''}`}>
            ...
            <StatCard label="Upcoming Elections" value={upcomingCount} icon={<Icons.Clock />} color="purple" />
            <StatCard label="Elections Completed" value={completedCount} icon={<Icons.Shield />} color="emerald" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
            ...
            <div className={`lg:col-span-2 space-y-6 opacity-0 ${mounted ? 'animate-enter delay-200' : ''}`}>
              ...
              {/* Dynamic Multiple Live Monitors */}
              {activeElections.length > 0 ? (
                <div className="space-y-6">
                  {activeElections.map((election) => {
                    const turnoutData = turnouts[election.election_id] || { total: 0, voted: 0, percentage: 0 };
                    
                    return (
                      <div key={election.election_id} className="relative overflow-hidden rounded-3xl bg-[#0a0a0a] border border-white/10 shadow-2xl group">
                        ...
                        <div className="p-8 relative z-10">
                          <div className="flex justify-between items-start mb-6">
                            <div>
                              ...
                              <h2 className="text-3xl font-bold text-white mb-2">{election.title}</h2>
                              <div className="flex items-center gap-4 text-sm text-gray-400">
                                <span className="flex items-center gap-1.5">
                                   <Icons.Users />
                                   {turnoutData.voted?.toLocaleString() || '0'} / {turnoutData.total?.toLocaleString() || '0'}
                                </span>
                          ...
                          <div className="space-y-4">
                            <div className="h-4 w-full bg-black/50 rounded-full overflow-hidden border border-white/10 p-0.5 shadow-inner">
                              <div 
                                ...
                                style={{ width: `${turnoutData.percentage || 0}%` }}
                              >
                        ...
                        <div className="bg-white/2 border-t border-white/5 p-4 flex justify-between items-center text-xs text-gray-500 font-mono">
                           <span>ID: #{election.election_id?.toString().padStart(4, '0')}</span>
                        </div>
                      </div>
                    );
                  })}
                         ...
                         {elections.slice(0, 5).map(e => {
                           ...
                           return (
                             <tr key={e.election_id} className="hover:bg-white/2 transition-colors group">
                                ...
                                <td className="px-6 py-4"><StatusBadge status={calcStatus} /></td>
                                <td className="px-6 py-4 text-gray-500 font-mono text-xs hidden sm:table-cell">{formatDate(e.end_date)}</td>
                             </tr>
                           );
                         })}
                         {elections.length === 0 && (
                           <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-600 italic">No elections records found</td></tr>
                         )}
                      </tbody>
                   </table>
                 </div>
